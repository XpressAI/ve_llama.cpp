/**
 * Optimized BF16 SGEMV kernel using LLVM-VE intrinsics
 * 
 * This is compiled separately with LLVM-VE, then linked with NCC for OpenMP support.
 * Based on sgemv-intrinsics by efocht.
 * 
 * Build: /usr/local/ve/llvm-ve-rv-2.2.0/bin/clang --target=ve-linux -O3 -fPIC -c ve_sgemv_bf16.c
 */
...
// Load BF16 weights and convert to packed FP32 for vector operations
...
// Sum up packed FP32 vector and store result
#define sumup_packed_fp32_store(tv, yt, VL) \
    do { \
...
// Optimized BF16 matrix-vector multiply with 16-way unrolling
// y = W @ x, where W is (d x n) in BF16, x is (n,) in FP32, y is (d,) in FP32
void sgemv_bf16_opt(float *y, float *x, bf16 *w, int n, int d) {
    ...
    int i;
    // 16-way unrolled for maximum throughput
    if (d >= 16) {
        for (i = 0; i < d - 15; i += 16) {
            __vr xv, xlv;
    ...
    for (; i < d; i++) {
        ...
        __vr tv1 = _vel_vld_vssl(0, &zero2[0], VLEN);
        bf16 *wp1 = w + i * n;
        for (int j = 0; j < n; j += 2 * VLEN) {
            const int vl = n - j < 2 * VLEN ? (n - j) >> 1 : VLEN;
            if ((unsigned long)(x + j) & 0x7) {
